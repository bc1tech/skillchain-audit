...
contract('SkillChainPrivateSale', function ([_, investor, wallet, purchaser, nextCrowdsaleAddress]) {
  const rate = new BigNumber(5000);

  const cap = ether(5);
  const lessThanCap = ether(3);
  const minimumContribution = ether(2);
  ...
  beforeEach(async function () {
    ...
    this.crowdsale = await SkillChainPrivateSale.new(this.startTime, this.endTime, rate, wallet, cap);

    this.token = SkillChainToken.at(await this.crowdsale.token());

    this.contributions = SkillChainContributions.at(await this.crowdsale.contributions());
  });
  ...
  describe('creating a valid crowdsale', function () {
    it('should fail with start time in the past', async function () {
      await SkillChainPrivateSale.new(
        latestTime() - duration.days(1),
        this.endTime,
        rate,
        wallet,
        cap).should.be.rejectedWith(EVMRevert);
    });
  ...
  describe('ended', function () {
    ...
    it('when closed should transfer token ownership to the presale contract', async function () {
      await increaseTimeTo(this.afterEndTime);
    ...
    it('when closed should transfer contributions ownership to the presale contract', async function () {
      await increaseTimeTo(this.afterEndTime);
      await this.crowdsale.closeTokenSale(nextCrowdsaleAddress);
